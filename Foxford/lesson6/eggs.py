...
def create_egg():
    x = randrange(10, 740)
    y = 40
    new_egg = c.create_oval(x, y, x + egg_width, y + egg_heigth, fill=next(color_cycle), width=0)
    eggs.append(new_egg)
...
def move_left(event):
    x1 = c.coords(catcher)[0]
    if x1 > 0:
        c.move(catcher, -20, 0)


def move_right(event):
    x2 = c.coords(catcher)[2]
    if x2 < canvas_width:
        c.move(catcher, 20, 0)
...
# задаем параметры яиц
color_cycle = cycle(["light blue", "light green", "light pink", "light yellow", "light cyan"])
egg_width = 45
egg_heigth = 55
egg_score = 10  # получаем 10 очков за каждое пойманное яйцо
egg_speed = 500
egg_interval = 4000  # новое яйцо возникает каждый 4000мс=4с
...
catcher_color = "blue"
catcher_width = 100
catcher_height = 100  # диаметр окружности для рисования дуги
# зададим начальную позицию корзины (у нижнего края по центру холста)
...
# game_font = font.nametofont('Timesnewroman')
# game_font.config(size=18)
score = 0
score_text = c.create_text(10, 10, anchor='nw', font='Timesnewroman', fill='darkblue', \
                           text='Счет: ' + str(score))
lives_remaining = 3
lives_text = c.create_text(canvas_width - 10, 10, anchor='ne', font='Timesnewroman', \
